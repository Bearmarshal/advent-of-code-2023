...
def part2(filename):
	...
	## field is the closest corner to or on the same row/column as the starting point, and the distance is just the Manhattan distance
	entry_points = dict()
	...
	for point, distance_from_start in entry_points.items():
		...
		if point == start:
			...
			print()
			print(f"reachable in one field = {len(distances_from_entry_point[congruence])}")
		else:
			...
			print()
			print(f"distance from start = {distance_from_start}, entered = {num_entered_fields}, fully = {num_full_fields}")
			# print(f"max distance from entry point = {max(distances_from_entry_point.values())}")
			if point.y != start.y and point.x != start.x:
				## Diagonals
				print(f"entered = {num_entered_fields * (num_entered_fields - 1) // 2}, fully = {num_full_fields * (num_full_fields - 1) // 2}")
				...
				for fields in range(num_full_fields, num_entered_fields):
					steps_remaining_at_field = steps_remaining - distance_from_start - fields * width
					congruence_index = congruence if fields % 2 == 0 else 1 - congruence
					num_reachable += (fields + 1) * len([steps for steps in distances_from_entry_point[congruence_index].values() if steps <= steps_remaining_at_field])
			else:
				print(f"entered = {num_entered_fields}, fully = {num_full_fields}")
